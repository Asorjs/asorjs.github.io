...
function getDefineProp(ts, sfc, edition) {
  ...
  function visitNode(node, parent, isReactivityTransform = false) {
    if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && [DEFINE_PROP, DEFINE_PROP_DOLLAR].includes(node.expression.escapedText)) {
      if (edition === "kevinEdition") {
        const type = node.typeArguments?.length ? getText(node.typeArguments[0], { ts, sfc }) : void 0;
        ...
        defineProps.push({
          name,
          prop: name,
          defaultValue: node.arguments.length > 0 ? getText(node.arguments[0], { ts, sfc }) : void 0,
          type: node.typeArguments?.length ? getText(node.typeArguments[0], { ts, sfc }) : void 0,
